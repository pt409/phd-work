...
C edited by: P. L. Taylor, pt409@cam.ac.uk
      module globals
         implicit none
...
ccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      common /pass1/ re(n),fe(n),rhoe(n),alpha(n),
     *   beta(n),beta1(n),A(n),B(n),cai(n),ramda(n),
     *   ramda1(n),Fi0(n),Fi1(n),Fi2(n),Fi3(n),
     *   Fm0(n),Fm1(n),Fm2(n),Fm3(n),Fm4(n),
     *   fnn(n),Fn(n),rhoin(n),rhoout(n),rhol(n),
     *   rhoh(n),rhos(n)
      common /pass2/ ielement(n),amass(n),Fr(5000,n),
     *   rhor(5000,n),z2r(5000,n,n),ntypes,blat(n),
